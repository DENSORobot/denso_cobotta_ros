...
void GripperBase::checkMotorState(void)
{
  ...
  try
  {
    ...
    if (motor_state)
      this->setMotorState(true);
    else
      this->setMotorState(false);
  }
...
// @return true is motor running
bool GripperBase::isMotorState(void) const
{
  return this->motor_state_;
}

void GripperBase::setMotorState(bool state)
{
  this->motor_state_ = state;
}
