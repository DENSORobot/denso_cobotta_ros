...
{
  ROS_INFO("%s: Linux driver: %d.%d.%d-%d (Requirements: %d.%d.N-M)", TAG,
           this->getVersion().driver_major, this->getVersion().driver_minor,
           this->getVersion().driver_revision, this->getVersion().driver_build,
...
/**
 * @brief Send parameters to all axes.
 *
 * Execute COBOTTA_IOCTL_CB_ACYCLIC_COMM.
 *
 ...
 * @exception RuntimeError An other error
 */
void Driver::sendParametersAll(uint16_t address, std::array<uint16_t, JOINT_MAX + 1> values) throw(CobottaException,
                                                                                                   std::runtime_error)
{
  writeHwAcyclicCommAll(parent_->getFd(), address, values);
}

/**
 * @brief Send parameters to one axis.
 *
 * Execute COBOTTA_IOCTL_CB_ACYCLIC_COMM.
 *
 * @param arm_no
 ...
 * @exception InvalidArgument arm_no is invalid.
 */
void Driver::sendParameters(long arm_no, long joint_no, uint16_t address,
                            uint16_t value) throw(CobottaException, std::runtime_error, std::invalid_argument)
{
  Driver::checkArmNo(arm_no);
  Driver::checkJointNo(joint_no);
  writeHwAcyclicComm(parent_->getFd(), arm_no, joint_no, address, value);
}
...
void Driver::writeHwClear(int fd) throw(CobottaException, std::runtime_error)
{
  ...
  myerrno = errno;
  ROS_DEBUG("%s: COBOTTA_IOCTL_CLR_ERROR ret=%d errno=%d result=%lX",
 ...
 * @exception RuntimeError An other error
 */
void Driver::writeHwAcyclicCommAll(int fd, uint16_t address,
                                                                                     ...
                                                                                     std::runtime_error)
{
  ...
  for (int j = 0; j < 9; j++)
  {
    ...
    dat.req_1_all.data[j].value = values[j];
  }
 ...
 * @exception RuntimeError An other error
 */
void Driver::writeHwAcyclicComm(int fd, long arm_no, long joint_no, uint16_t address,
...
long Driver::readHwGripperState(int fd) throw(CobottaException, std::runtime_error)
{
...
void Driver::checkJointNo(long joint_no) throw(std::invalid_argument)
{
  ...
  {
    throw std::invalid_argument("Invalid joint_no: " + std::to_string(joint_no) +
                                " is out of range(0 <= joint_no < " + std::to_string(JOINT_MAX) + ").");
  }
}
