...
public:
  ...
  int openDeviceFile() noexcept(false);
  ...
  void clearError() noexcept(false);
  StateCode dequeue(long arm_no) noexcept(false);
  DriverStateInfo receiveAllState() noexcept(false);
  uint32_t getStateQueueSize(long arm_no) const noexcept(false);
  DriverVersion getVersion() const;
  bool isError() const;
  bool isFatalError() const;
  bool isIpResetButtonOn() const;

  static void checkArmNo(long arm_no) noexcept(false);
  static void checkJointNo(long joint_no) noexcept(false);

  static struct DriverCommandInfo writeHwUpdate(int fd, const SRV_COMM_SEND& send_data) noexcept(false);
  static SRV_COMM_RECV readHwEncoder(int fd, long arm_no) noexcept(false);
...
private:
  static DriverVersion readHwVersion(int fd) noexcept(false);
  static void writeHwClear(int fd) noexcept(false);
  static StateCode readHwQueue(int fd, long arm_no) noexcept(false);
  static DriverStateInfo readHwState(int fd) noexcept(false);
